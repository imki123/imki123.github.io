...
function App() {
	const homeText = `행복한 세상 블로그에 오신 것을 환영합니다 :D`
	const aboutText = `안녕하세요. 초보 웹 프로그래머입니다. 프로그래밍 관련 블로그를 제작중입니다. 주요 관심분야는 React, Koa, Node, JS입니다.
  서둘러 블로그를 이쁘게 꾸미고 싶네요. 조금만 기다려주세요~ `
	const [
		articleText,
		setArticleText,
	] = useState(`끓는 새가 가치를 이상의 기쁘며, 심장의 귀는 위하여 내는 것이다. 청춘의 넣는 돋고, 그들에게 피어나는 얼마나 무엇을 피다. 때까지 들어 행복스럽고 황금시대의 봄바람이다. 눈이 꽃이 하는 피어나는 그들에게 인생에 넣는 안고, 것이다. 석가는 위하여, 붙잡아 우리의 황금시대다. 설산에서 그것을 있음으로써 심장은 부패를 귀는 것이다. 긴지라 현저하게 물방아 노래하며 보이는 뜨고, 위하여서. 미묘한 청춘은 평화스러운 커다란 있는 긴지라 소리다.이것은 봄바람이다. 살 때까지 찾아 그들의 고행을 위하여 봄바람이다.
